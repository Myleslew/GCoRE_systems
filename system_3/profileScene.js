    ...
    showDashboard() {
        ...
        signOutButton.addEventListener('click', () => {
            if (confirm('Are you sure you want to sign out?')) {
                // Create and download profile data as JSON file
                const profileData = window.ProfileManager.currentProfile;
                const blob = new Blob([JSON.stringify(profileData, null, 2)], { type: 'application/json' });
                const url = URL.createObjectURL(blob);
                const a = document.createElement('a');
                a.href = url;
                a.download = `profile_${profileData.username}_${new Date().toISOString().split('T')[0]}.json`;
        ...
        // Add Rubric Guide button
        const rubricButton = document.createElement('button');
        rubricButton.textContent = 'View Rubric Guide';
        rubricButton.style = `
            padding: 12px 24px;

    ...
    window.EvaluationScene = class EvaluationScene extends Phaser.Scene {
        ...
        showEvaluationScreen() {
            ...
            submitButton.addEventListener('click', () => {
                ...
                // Convert scores to the format expected by GamificationManager
                const studentGrades = {};
                let totalInputScore = 0;
                Array.from(this.scores.entries()).forEach(([criterionIndex, score]) => {
                    const criterionName = window.CodeRubric.categories[0].criteria[criterionIndex].name;
                    console.log('Converting criterion:', criterionName); // Add logging
                    studentGrades[criterionName] = score;
                    totalInputScore += score;
                });

                // Calculate points using GamificationManager
                ...
                // Save the evaluation with the total input score
                if (window.ProfileManager.saveEvaluation(
                    ...
                    successMessage.textContent = `Review submitted successfully! You earned ${totalInputScore} points.`;
